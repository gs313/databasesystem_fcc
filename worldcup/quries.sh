...
echo "$($PSQL "SELECT name FROM games FULL JOIN teams ON games.winner_id = teams.team_id OR games.opponent_id = team_id WHERE year=2014 AND round='Eighth-Final' ORDER BY name")"
...
echo "$($PSQL "SELECT DISTINCT(name) FROM games FULL JOIN teams ON games.winner_id = teams.team_id OR games.opponent_id = team_id WHERE name LIKE 'Co%' ORDER BY name")"
